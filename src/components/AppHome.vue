<template>
	<div class="hello">
		<header class="header">
			<div class="fix_nav">
				<div style="max-width:768px;margin:0 auto;height: 44px;position: relative;background:#000000;">
					<form action="/m_search/list" method="get" id="searchform" name="searchform">
						<div class="navbar-search left-search">
								<input type="text" id="keyword" name="keyword" value="" placeholder="搜索商品" class="form-control">
						</div>
						<div class="nav-right">
							<input type="button" value="搜索" onclick="searchproduct();" class="img-responsive" style="text-align:center;background:#ccc;border-radius: 5px;border:none;height:34px;vertical-align:middle;clear:both;padding:0px;width:42px;"/>
						</div>
					</form>
				</div>
			</div>
		</header>

		<div class="container">
		<div class="row">
			<app-slide></app-slide>
		</div>
		<div class="row category">
			<a href="/m_search/list?param.hot='Y'" class="col-xs-3">
				<img class="img-responsive" src="../assets/img/icon_rm.png">
				<h4>热门</h4>
			</a>
			<a href="/m_search/list?param.commend='Y'" class="col-xs-3">
				<img class="img-responsive" src="../assets/img/icon_tm.png">
				<h4>精品</h4>
			</a>
			<a href="/theme/allThemes" class="col-xs-3">
				<img class="img-responsive" src="../assets/img/theme.png">
				<h4>专题列表</h4>
			</a>
			<a href="/brand" class="col-xs-3">
				<img class="img-responsive" src="../assets/img/icon_pp.png">
				<h4>品牌</h4>
			</a>
		</div>
		
		<div class="row">
			<!--产品块-->
			<div class="tb_box">
				<h2 class="tab_tit">
					<a class="more" href="http://m.legendshop.cn/m_search/list?categoryId=36">更多</a>
					礼品箱包</h2>
				
				<div class="tb_type tb_type_even clearfix">
					<router-link to="/views" class="tb_floor">
						<img src="../assets/img/c2abb958-aca8-42f2-be52-ee1c072caec0.jpg" style="width:100%;">
					</router-link>
					<router-link to="/views" class="th_link">
						<img class="tb_pic" src="../assets/img/e71099af-8a08-45f5-af16-4e8edf2f1bff.jpg" style="width:100%;">
					</router-link>
					<router-link to="/views" class="th_link tb_last">
						<img class="tb_pic" src="../assets/img/33c256a8-7bff-4c6b-8686-f209bbad1301.jpg" style="width:100%;">
					</router-link>
				</div>
			</div>
			<!--产品块-->
			<div class="tb_box">
				<h2 class="tab_tit">
					<a class="more" href="http://m.legendshop.cn/m_search/list?categoryId=38">更多</a>
					服饰鞋帽</h2>
				
				<div class="tb_type clearfix">
					<router-link to="/views" class="tb_floor">
						<img src="../assets/img/0bbbb6bf-0d00-45c0-92f7-347377f2edb5.jpg" style="width:100%;">
					</router-link>
					<router-link to="/views" class="th_link">
						<img class="tb_pic" src="../assets/img/0d86960d-20b5-4dd3-afee-8453b5ea5e95.jpg" style="width:100%;">
					</router-link>
					<router-link to="/views" class="th_link tb_last">
						<img class="tb_pic" src="../assets/img/1e13498f-419d-4ebd-a3b2-d0ad95ceaa39.jpg" style="width:100%;">
					</router-link>
				</div>
			</div>
			<!--产品块-->
			<div class="tb_box">
				<h2 class="tab_tit">
					<a class="more" href="http://m.legendshop.cn/m_search/list?categoryId=35">更多</a>
					数码办公</h2>
				
				<div class="tb_type tb_type_even clearfix">
					<router-link to="/views" class="tb_floor">
						<img src="../assets/img/bd6771d9-f220-454d-83b2-d58d2046d23a.jpg" style="width:100%;">
					</router-link>
					<router-link to="/views" class="th_link">
						<img class="tb_pic" src="../assets/img/04ef50d9-97f0-4fea-8359-ee21376df392.jpg" style="width:100%;">
					</router-link>
					<router-link to="/views" class="th_link tb_last">
						<img class="tb_pic" src="../assets/img/d07a93ef-cdff-4b21-a12b-d817890aa6d6.jpg" style="width:100%;">
					</router-link>
				</div>
			</div>
			</div>
		</div>

		<div class="foot_index">
			<div class="foot_index_tit">Aim Beauty Limited(HongKong)</div>
			<div class="foot_index_rx">服务热线：020-87774389</div>
		</div>
	</div>
</template>
<script>
import AppSlide from '../common/AppSlide';
export default {
	components: { AppSlide}
}
</script>




