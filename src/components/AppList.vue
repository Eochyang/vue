...
<div class="container" id="container2">
	<div class="row">
		...
		<div class="item-list" id="container" rel="2" status="0"><input type="hidden" id="ListTotal" value="1">
			<router-link :to="'/views/'+item.goodsId" tag="li" v-for="item of list" :key="item.id">
				<div class="hproduct clearfix" style="background:#fff;border-top:0px;">
					<div class="p-pic">
						<img style="max-height:100px;margin:auto;" class="img-responsive" :src="item.img">
					</div>
					<div class="p-info">
						<p class="p-title">{{item.name}}</p>
						<p class="p-origin"><em class="price">{{item.newprice}}</em>
</p>
						<p class="mb0"><del class="old-price">{{item.oldprice}}</del></p>
					</div>
				</div>
			</router-link>
